	...
	with(otherSpeaker) {
		with(target) {
			...
			with(_opinionData) {
				...
				energy += energvspeed * individualEffectMult * (1 / power(abs(energy - .5) * 2 + 1, 2.0));
			}
			...
			emotionAnger += angerChange * (1 / power(abs(emotionAnger - .5) * 2 + 1, 2.0));
			emotionFear += fearChange * (1 / power(abs(emotionFear - .5) * 2 + 1, 2.0));
			emotionEnergy += energvspeed * (1 / power(abs(emotionEnergy - .5) * 2 + 1, 2.0));
